<?php
    ...
    for($i = 0; $i < mysqli_num_rows($result1); $i++){
        ...
        if ($currentCategory !== $row_cat['name']) {
              ...
              for($n = 0;($n< (5 - $i%5)) &&($i%5!=0); $n++) $code11 .= '<td style="border: 0.5px solid rgba(0, 0, 0, 0.5); padding: 2.5px; width: 20%;"></td>';
              ...
              for($n = 0; $n < $i%5 ;$n++) $code11 .= '<td style="border: 0.5px solid rgba(0, 0, 0, 0.5); padding: 2.5px; width: 20%;"></td>';
         }
    ...
    // 1-1-sequence
    $query2 = "SELECT * FROM code WHERE type = '2' ORDER BY sequence";
	...
	// MP
    $query4 = "SELECT * FROM code WHERE type = '4' ORDER BY sequence";
    ...
    for($i = 0; $i < mysqli_num_rows($result4); $i++){
        ...
        $codeMP .= '<tr><td style="padding-top: 8px;"><small class="my-0 py-0" style="font-size: 10px;">MP : '.$row_char[$i].'</small></td></tr><tr>';
        ...
        for($j = 0; $j < sizeof($file_MP); $j++){
			...
			<td style="border: 1px solid rgba(0, 0, 0, 0.5); padding: 1px; margin: 0px; width: 20%;">
                ...
                <small class="text-muted my-0 py-0" style="width: 100%;  padding: 0px; margin: 0px;">'.str_pad($j+1, 2, "0", STR_PAD_LEFT).'</small>

                <img src="code/MP/'.$file_MP[$j].'.bmp" style="image-rendering: pixelated; width:100%; padding: 0px; margin: 0px;">
        ...
        for($n = 0; ($n < (5 - $j%5)) && ($j%5!=0); $n++) $codeMP .= '<td style="border: 0.5px solid rgba(0, 0, 0, 0.5); padding: 2.5px; width: 20%;"></td>';
	...
	// BAKA
    $query5 = "SELECT * FROM code WHERE type = '5' ORDER BY sequence";
    ...
    for($i = 0; $i < mysqli_num_rows($result5); $i++){
        ...
        for($j = 0; $j < sizeof($file_BAKA); $j++){
			...
			<td style="border: 1px solid rgba(0, 0, 0, 0.5); padding: 1px; margin: 0px; width: 20%;">
                ...
                <small class="text-muted my-0 py-0" style="width: 100%; padding: 0px; margin: 0px;">'.$row_char_baka[$j].'</small>

                <img src="code/BAKA/'.$file_BAKA[$j].'.bmp" style="image-rendering: pixelated; width:100%; padding: 0px; margin: 0px;">
        ...
        for($n = 0; ($n < (5 - $j%5)) && ($j%5!=0); $n++) $codeBAKA .= '<td style="border: 0.5px solid rgba(0, 0, 0, 0.5); padding: 2.5px; width: 20%;"></td>';
